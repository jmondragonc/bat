...
if(isset($_GET['fbid']) && !empty($_GET['fbid']) && isset($_GET['userid']) && !empty($_GET['userid'])){
    ...
    if(isset($_GET['page']) && !empty($_GET['page'])){
        header("Location: galeria?page=".$_GET['page']);
    }else{
...
if(isset($_GET['id']) && !empty($_GET['id'])){
    if(isset($_SESSION['userid'])){
        ...
        WHERE i.status=1 AND ISNULL(i.yaparticipo) AND i.fecha >= DATE('2018-04-01') AND i.id=%i ", $_GET['id']);
    }else{
        $compartir = DB::queryFirstRow("SELECT *, (select count(*) FROM voto v WHERE v.imgid=i.id) AS totalvotos, 
        i.id as idimg 
        FROM imagen i 
        INNER JOIN usuario u on u.id = i.userid 
        WHERE i.status=1 AND ISNULL(i.yaparticipo) AND i.fecha >= DATE('2018-04-29') AND i.id=%i ", $_GET['id']);
    }
}
...
if(isset($_SESSION['userid'])){
    ...
    {
        header("Location: galeria?b=".urlencode($_POST['buscar']));
    }
    ...
    if(isset($_GET['b']) && $_GET['b']!='')
    {
        ...
        $imgs = DB::query("SELECT *, (select count(*) from voto v WHERE v.imgid=i.id) AS totalvotos, 
            ...
            FROM imagen i
            INNER JOIN usuario u on u.id = i.userid 
            WHERE (i.status=%i AND ISNULL(i.yaparticipo) AND i.fecha >= DATE('2018-04-01')) AND (u.nombres ".$query_string.")", 1);
        ...
        $results = DB::query("SELECT *, (select count(*) from voto v WHERE v.imgid=i.id) AS totalvotos, 
            ...
            FROM imagen i
            INNER JOIN usuario u on u.id = i.userid 
            WHERE (i.status=%i AND ISNULL(i.yaparticipo) AND i.fecha >= DATE('2018-04-29')) AND (u.nombres ".$query_string.")  
    ...
    else
    {
        ...
        else
        {
            ...
            $results = DB::query("SELECT *, (select count(*) from voto v WHERE v.imgid=i.id) AS totalvotos, 
                    ...
                    FROM imagen i
                    INNER JOIN usuario u on u.id = i.userid 
                    WHERE i.status=%i AND ISNULL(i.yaparticipo) AND i.fecha >= DATE('2018-04-29') 
                    LIMIT $offset, $rowsperpage", 1);
            $counter_res = DB::count();
    ...
    if(isset($_GET['b']) && $_GET['b']!='')
        {
            ...
            FROM imagen i
            INNER JOIN usuario u on u.id = i.userid 
            WHERE (i.status=%i AND ISNULL(i.yaparticipo) AND i.fecha >= DATE('2018-04-29')) AND (u.nombres ".$query_string.")", 1);
            ...
            <div class="page">
                ...
                <div class="gal">
                    <div class="info">
                        ...
                        <a href="participantes" class="mob-btn mobile">VER TODAS LAS FOTOS</a>
                        <div class="row head">
                            ...
                            <div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 desktop"><a href="galeria" class="ltop<?php if( !isset($_GET['rnk']) ){ echo ' active'; } ?>">ORDEN DE INGRESO</a></div>
                            <div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 desktop"><a href="galeria?rnk=ok" class="ltop<?php if( isset($_GET['rnk']) && $_GET['rnk']=='ok' ){ echo ' active'; } ?>">TOP RANKING</a></div>
                            <div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 desktop"><a href="participantes" class="btn">VER TODAS LAS FOTOS</a></div>
                        ...
                        <?php
                        if($counter_res>0 && $totalpages>1){?>
                        <div class="row pagination">
                            <div class="col-xs-2 col-sm-2 col-md-2 col-lg-2">
                                <?php
                                if(isset($_GET['rnk']) && $_GET['rnk']=='ok'){
                                    $rnk = '&rnk=ok';
                                } else {
                                    $rnk='';
                                }
                                ...
                                if ($currentpage > 1) {
                                    ...
                                    echo " <a href='{$_SERVER['PHP_SELF']}?page=$prevpage$rnk' class='pageprev'></a> ";
                                } ?>
                            </div>
                            <div class="col-xs-7 col-sm-7 col-md-8 col-lg-8 mobile"></div>
                            <div class="col-xs-8 col-sm-8 col-md-8 col-lg-8 links desktop">
                                <?php
                                ...
                                for ($x = ($currentpage - $range); $x < (($currentpage + $range) + 1); $x++) {
                                    if (($x > 0) && ($x <= $totalpages-3)) {
                                        if ($x == $currentpage) {
                                            echo " <span class='curpage'>$x</span>";
                                        } else {
                                            echo " <a href='{$_SERVER['PHP_SELF']}?page=$x$rnk'>$x</a> ";
                                        }
                                    }
                                }
                                $antpen = $totalpages-2;
                                $pen = $totalpages-1;
                                    ...
                                    if($pen-8>=$currentpage){
                                        echo "<span class='dots'>...</span>";
                                    }

                                    if($currentpage==$antpen){
                                        echo " <span class='curpage'>$antpen</span>";
                                    }else{
                                        echo " <a href='{$_SERVER['PHP_SELF']}?page=$antpen$rnk'>$antpen</a> ";
                                    }
                                    if($currentpage==$pen){
                                        echo " <span class='curpage'>$pen</span>";
                                    }else {
                                        echo " <a href='{$_SERVER['PHP_SELF']}?page=$pen$rnk'>$pen</a> ";
                                    }
                            ...
                            <div class="col-xs-2 col-sm-2 col-md-2 col-lg-2 right">
                                ...
                                if ($currentpage < $totalpages-1) {
                                    ...
                                    if($totalpages>1){
                                        echo " <a href='{$_SERVER['PHP_SELF']}?page=$nextpage$rnk' class='pagenext'></a> ";
