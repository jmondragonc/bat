...
if(isset($_POST['form']) && !empty($_POST['form'])){
    ...
    {
        header("Location: fotos_eliminadas.php?b=".urlencode($_POST['buscar']));
    }
    ...
    if(isset($_GET['b']) && $_GET['b']!='')
    {
        ...
        $imgs = DB::query("SELECT *, i.id AS idimg 
        FROM imagen i 
        INNER JOIN usuario u ON i.userid = u.id 
        WHERE i.status=%i AND u.nombres ".$query_string, 2);
        ...
        $results = DB::query("SELECT *, i.id AS idimg 
        FROM imagen i 
        INNER JOIN usuario u ON i.userid = u.id 
        WHERE i.status=%i AND u.nombres ".$query_string." 
        LIMIT $offset, $rowsperpage", 2);
    ...
    else
    {
        ...
        {
            //Query por defecto
            $imgs = DB::query("SELECT *, i.id AS idimg, i.fecha AS fechaimg FROM imagen i INNER JOIN usuario u ON i.userid = u.id WHERE i.status=%i", 2);
            $counter = DB::count();
            $numrows = $counter;
            $rowsperpage = 20;
            $totalpages = ceil($numrows / $rowsperpage);
  ...
  <div class="content-wrapper">
    <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Fotos eliminadas (<?php echo $counter; ?>)</li>
      </ol>
      <!-- Example DataTables Card-->
      <div class="card mb-3">
        <div class="card-header">
          <i class="fa fa-table"></i> Fotos eliminadas recientemente</div>
        <div class="card-body">
          <div class="table-responsive">
              <div class="row">
                  <div class="col-md-12 text-right busqueda">
                      <form name="busqueda" id="busqueda" action="fotos_eliminadas.php" method="post">
              ...
              <tbody>
                ...
                <?php } } ?>
                  ...
                  <div style="height: 30px;"></div>
                  <div class="card-footer small text-muted text-right"><input type="submit" class="btn btn-primary" value="RESTAURAR" name="submit"></div>
